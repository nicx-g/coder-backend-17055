...
router.get("/productos/listar", function (req, res) {
  ...
  if (arrayOfProducts.error) res.status(404).json({
    error: arrayOfProducts.error
...
router.get("/productos/listar/:id", function (req, res) {
  ...
  if (product.error) res.status(404).json({
    error: product.error
...
router.put("/productos/actualizar/:id", function (req, res) {
  ...
  if (updatedProduct.error) res.status(404).json({
...
router["delete"]("/productos/borrar/:id", function (req, res) {
  ...
  if (removedProduct.error) res.status(404).json({
